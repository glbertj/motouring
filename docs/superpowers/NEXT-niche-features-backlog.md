...
**Last updated:** 2026-07-16, after Spec 7 (Rider Stats & Insights) shipped to `origin/main`. **All four niche-feature themes, the one deferred sub-idea (scenic routes), AND the first post-backlog new feature (insights dashboard) are now DONE — nothing queued.**
...
9. **Rider Stats & Insights (Spec 7)** — the first **post-backlog new feature** (requested directly, not from this backlog). A data-viz dashboard over ride history reached from **Profile → "Insights"**: lifetime totals + a weekly-distance bar chart + personal records + a moto-vs-car split bar + a ride-score sparkline. Pure `simulation/InsightsCalculations.kt` (`lifetimeTotals`/`weeklyDistanceKm`/`personalRecords`/`vehicleSplit`/`rideScoreTrend`, TDD'd), a snapshot `InsightsViewModel`, three Compose-`Canvas` charts in `ui/insights/InsightsCharts.kt` (all divide-by-zero-guarded, brand-tokens-only), `InsightsScreen`, and `Destinations.INSIGHTS` wiring. `FakeDataProvider.rideHistory` grown 3→12 entries across 12 weeks. Data-viz method applied (single accent for single-series; categorical split validated CVD-excellent + direct-labeled). Spec + plan in `docs/superpowers/`. **On-device visual pass still pending on the Arch host.** Minors logged (all cosmetic): charts consolidated in one file vs the spec's three; no x-axis week ticks (peak shown as a caption).
