...
**Last updated:** 2026-07-14, after Spec 3 (Safety & SOS) shipped to `origin/main`.
...
> Read `docs/superpowers/NEXT-niche-features-backlog.md`. Let's build the next theme: **Gear & maintenance**. Brainstorm it first (use the visual companion for layout questions), then spec → plan → subagent-driven build with Sonnet 5 doing the coding, direct-to-main like last time.
...
Five bodies of work are DONE and on `main`:
...
### 3. Gear & maintenance  ← DO THIS NEXT
