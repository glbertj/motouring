...
The current placeholder vector drawables (`ic_avatar_placeholder`, `ic_vehicle_motorcycle_placeholder`, `ic_vehicle_car_placeholder`, `ic_route_preview_placeholder`, `ic_photo_placeholder`, `ic_badge_placeholder`) are generic single-path Material stock glyphs — visually inconsistent with a considered gauge/mono-numeral aesthetic. These 6 are data-bound: `Badge.iconRes`, `Vehicle.photoRes`, `RideHistoryEntry.routePreviewRes`, `Post.photoResList`, and `User.avatarRes` are plain `Int` drawable-resource fields on the data models, so their replacements must stay as XML vector drawables under the same resource names (redrawn in place) — turning them into arbitrary Compose composables would require touching the data models, which is out of scope.

This phase redraws those 6 placeholders as a consistent thin-stroke geometric line-icon set (rounded caps/joins, matching Space Grotesk's geometric character), same file names, zero call-site changes required elsewhere.

The ad-hoc `Icons.Filled.*` Material glyphs used for UI chrome (bottom-nav tabs, like/comment/send) are **not** data-bound and are deliberately left as-is rather than hand-authoring a full custom `ImageVector` set — they already re-tint and re-size consistently once the theme's color scheme and a shared icon-sizing convention are in place, and hand-drawing a dozen more icon paths without any visual iteration tooling (unlike the palette/motion choices validated in the browser companion) is a lot of unverified risk for something not explicitly asked for. Worth a dedicated follow-up pass if it turns out to matter once the rest of the system is visible on-device.

## Rollout Scope

Verified against the current repo: 20 screens are actually built and restyled in this phase — pre-auth (Splash, Onboarding, Login, Vehicle Garage Setup), Home, Social (Create Post, Post Detail, Friends, Invite Ride), Challenges (Challenges, Challenge Detail, Badges, Badge Detail), Rides (Rides History, Start Ride, Ride Summary), and Profile (Profile, Edit Profile, Settings, Notifications) — plus every shared component (`StatBlock`, `PostCard`, `RideBuddyAvatarRow`, `BadgeChip`, `SectionHeader`, bottom nav, top bars). `ui/theme/*` is rebuilt from scratch, shared components are rebuilt against the new tokens (with `InstrumentRing` replacing progress bars wherever one appears), then every screen is re-themed and given its entrance/press motion. Compose `@Preview`s are updated alongside each component/screen they cover.

**Nearby and Ride Session do not exist as screens yet** — they're still-unstarted tasks from the original plan, blocked on the Mapbox token (Mapbox isn't in `build.gradle.kts` yet either). There's no existing chrome to restyle on either of them, so they are fully out of scope for this phase, not partially restyled. When they're eventually built, they should be built directly against the tokens this phase produces. Ride Summary *does* exist already (it only needs the static route-preview drawable, no live Mapbox dependency) and is in scope for full restyle.
